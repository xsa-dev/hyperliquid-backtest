//! Unit tests for the HyperliquidDataFetcher
...
// Additional tests for the unified data fetching interface (Task 4.3)
#[cfg(test)]
mod unified_fetch_tests {
    ...
    fn test_max_time_ranges_for_intervals() {
        let test_cases = [
            ("1m", 7 * 24 * 3600),      // 1 week
            ("5m", 30 * 24 * 3600),     // 1 month
            ("15m", 90 * 24 * 3600),    // 3 months
        ...
        for (interval, expected_max) in test_cases.iter() {
            ...
            // Use a reasonable historical start time that won't trigger future time validation
            let start_time = 1609459200; // 2021-01-01 00:00:00 UTC - well in the past
            
            // For very large ranges like 1d (5 years), use a smaller test range to avoid future time issues
            let test_range = if *expected_max > 2 * 365 * 24 * 3600 {
                // For 1d interval, test with 1 year instead of 5 years to avoid future time validation
                365 * 24 * 3600
            } else {
                max_range - 1
            };
            ...
            // Test that a range over the actual limit is invalid
    ...
    fn test_edge_cases_for_time_validation() {
        ...
        // Test very old timestamps (should be valid as long as range is reasonable)
        let old_start = 1000000000; // Year 2001
        let old_end = old_start + 3600; // 1 hour later
        let result = HyperliquidData::validate_fetch_parameters("BTC", "1h", old_start, old_end);
        assert!(result.is_ok());
    }
    ...
    fn test_comprehensive_parameter_combinations() {
        ...
        for coin in &coins {
            for interval in &intervals {
                // Test with appropriate time range for each interval
                let max_range = HyperliquidDataFetcher::max_time_range_for_interval(interval);
                let safe_range = max_range / 2; // Use half the maximum range to be safe
                let end_time = base_time + safe_range;
